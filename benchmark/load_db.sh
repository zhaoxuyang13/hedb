...
# for i in `ls $data_dir/*.tbl`; do
#     tbl_name=`basename $i .tbl`
#     #  copy 0from stdin to load *tbl files to postgresql database with delimiter '|'
#     # echo "copy $tbl_name from stdin with delimiter '|';"
#     cat $i | psql -U $db_user -h $db_host -p $db_port -d $db_name -c "copy $tbl_name () from stdin with delimiter '|';"
