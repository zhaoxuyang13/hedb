...
int enc_text_like(uint8_t* in1,
                  ...
                  size_t out_size)
{
    int str_raw_size = (in1_size - SGX_AESGCM_IV_SIZE - SGX_AESGCM_MAC_SIZE)/2;
    int pattern_raw_size = (in2_size - SGX_AESGCM_IV_SIZE - SGX_AESGCM_MAC_SIZE)/2;
    int resp, result;
...
int enc_text_concatenate(uint8_t* string1,
                         ...
                         size_t string3_len)
{
    int raw_str1_len = (string1_len - SGX_AESGCM_IV_SIZE - SGX_AESGCM_MAC_SIZE)/2;
    int raw_str2_len = (string2_len - SGX_AESGCM_IV_SIZE - SGX_AESGCM_MAC_SIZE)/2;
    int raw_str3_len = (string3_len - SGX_AESGCM_IV_SIZE - SGX_AESGCM_MAC_SIZE)/2;
    int resp;

    uint8_t* dec_string = (uint8_t*)malloc(raw_str3_len + 1);

    resp = decrypt_bytes(string1, string1_len, dec_string, raw_str1_len);
    if (resp != SGX_SUCCESS)
        return resp;

    resp = decrypt_bytes(string2, string2_len, dec_string + raw_str1_len, raw_str2_len);
    if (resp != SGX_SUCCESS)
        return resp;

    resp = encrypt_bytes(dec_string, raw_str3_len, string3, string3_len);

    memset_s(dec_string, raw_str3_len + 1, 0, raw_str3_len + 1);

    free_allocated_memory(dec_string);
...
int enc_text_substring(uint8_t* in1,
                       ...
                       size_t* out_size)
{
    size_t str_size = (in1_size - SGX_AESGCM_IV_SIZE - SGX_AESGCM_MAC_SIZE)/2;
    ...
    *out_size = n_chars.val*2 + SGX_AESGCM_IV_SIZE + SGX_AESGCM_MAC_SIZE;
