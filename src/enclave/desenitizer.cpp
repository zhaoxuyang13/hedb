...
static int retrieve_request_from_file(FILE *f, char* req_buffer){
    ...
    br->reqType = op;
    switch (op)
    { // here each buffer should decrease by an int(which is op, that is read earlier)
        case CMD_INT_CMP:{
        ...
        case CMD_STRING_SUBSTRING:{
            ...
            src += str_length;
            memcpy(&req->begin, src, ENC_INT32_LENGTH);
            src += ENC_INT32_LENGTH;
            memcpy(&req->end, src, ENC_INT32_LENGTH);
            src += ENC_INT32_LENGTH;
            break;
        }
        case CMD_STRING_CONCAT:{
