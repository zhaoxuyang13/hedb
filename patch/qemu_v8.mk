...
QEMU_SMP 	?= 4,cores=2,threads=2,sockets=1
QEMU_MEM 	?= 2G # changed 
QEMU_VIRT	= false
endif
...
run-only:
	ln -sf $(ROOT)/out-br/images/rootfs.cpio.gz $(BINARIES_PATH)/
	ln -sf $(ROOT)/img/ramdisk $(BINARIES_PATH)/ubuntu.img
	$(call check-terminal)
	$(call run-help)
	...
	cd $(BINARIES_PATH) && $(QEMU_BUILD)/aarch64-softmmu/qemu-system-aarch64 \
		...
		$(QEMU_XEN) \
		$(QEMU_EXTRA_ARGS) \
		#-device e1000,netdev=network0,mac=06:68:9f:d3:9f:e1 \
		# -netdev tap,id=network0,ifname=tap0,script=no,downscript=no  
# -net hostfwd=tcp::15432-:5432
